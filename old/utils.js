const rgbArrayToCss = rgbArray => `rgb(${rgbArray[0]}, ${rgbArray[1]}, ${rgbArray[2]})`;
...
function deltaE2000(Lab1, Lab2) {
    ...
    // Calculate C1, C2, and Cbar
    const C1 = Math.sqrt(Lab1[1] * Lab1[1] + Lab1[2] * Lab1[2]);
    const C2 = Math.sqrt(Lab2[1] * Lab2[1] + Lab2[2] * Lab2[2]);
    const Cbar = (C1 + C2) / 2;
  
    // Calculate a' and C'
    const a1Prime = Lab1[1] + (Lab1[1] / 2) * (1 - Math.sqrt(Math.pow(Cbar, 7) / (Math.pow(Cbar, 7) + Math.pow(25, 7))));
    const a2Prime = Lab2[1] + (Lab2[1] / 2) * (1 - Math.sqrt(Math.pow(Cbar, 7) / (Math.pow(Cbar, 7) + Math.pow(25, 7))));
    const C1Prime = Math.sqrt(a1Prime * a1Prime + Lab1[2] * Lab1[2]);
    const C2Prime = Math.sqrt(a2Prime * a2Prime + Lab2[2] * Lab2[2]);
    const CbarPrime = (C1Prime + C2Prime) / 2;
  
    // Calculate h' and Δh'
    const G = 0.5 * (1 - Math.sqrt(Math.pow(CbarPrime, 7) / (Math.pow(CbarPrime, 7) + Math.pow(25, 7))));
    const a1PrimeDeg = (Math.atan2(Lab1[2], a1Prime) * 180) / Math.PI;
    const a2PrimeDeg = (Math.atan2(Lab2[2], a2Prime) * 180) / Math.PI;
    const h1Prime = a1PrimeDeg + (a1PrimeDeg >= 0 ? 0 : 360);
    const h2Prime = a2PrimeDeg + (a2PrimeDeg >= 0 ? 0 : 360);
    const deltaHPPrime = Math.abs(h1Prime - h2Prime) <= 180 ? h2Prime - h1Prime : (h2Prime <= h1Prime ? h2Prime - h1Prime + 360 : h2Prime - h1Prime - 360);
  
    // Calculate ΔL', ΔC', and ΔH'
    const deltaLPrime = Lab2[0] - Lab1[0];
    const deltaCPrime = C2Prime - C1Prime;
    const deltaHPrime = 2 * Math.sqrt(C1Prime * C2Prime) * Math.sin((deltaHPPrime * Math.PI) / 180);
  
    // Calculate Lbar', Cbar', and hbar'
    const LbarPrime = (Lab1[0] + Lab2[0]) / 2;
    const CbarPrimePow7 = Math.pow(CbarPrime, 7);
    const h1PrimeDegAbs = Math.abs(h1Prime - 360);
    const h2PrimeDegAbs = Math.abs(h2Prime - 360);
    ...
    const deltaTheta = 30 * Math.exp(-((hbarPrime - 275) / 25) * ((hbarPrime - 275) / 25));
    const R = 2 * Math.sqrt(CbarPrimePow7 / (CbarPrimePow7 + 6103515625));
    const SL = 1 + ((0.015 * (LbarPrime - 50) * (LbarPrime - 50)) / Math.sqrt(20 + (LbarPrime - 50) * (LbarPrime - 50)));
    const SC = 1 + 0.045 * CbarPrime;
    const SH = 1 + 0.015 * CbarPrime * T;
    const RT = -Math.sin((2 * deltaTheta) * (Math.PI / 180)) * R;
